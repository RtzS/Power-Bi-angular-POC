...
import { HttpService, TokenServiceResponse } from './services/http.service';
...
export class AppComponent {
  ...
  @ViewChild('airlineLabel')
  private airlineLabelRef!: ElementRef<HTMLLabelElement>;
  @ViewChild('airlineField')
  private airlineFieldRef!: ElementRef<HTMLInputElement>;
  ...
  async embedReport(): Promise<void> {
    const airlineId = this.airlineFieldRef.nativeElement.value;
    let url =
      'http://localhost:8080/token-generator/airline/' + airlineId + '/token';

    var tokenResponse = await firstValueFrom(
      this.httpClient.post<TokenServiceResponse>(url, null)
    );
  ...
  async prepareDisplayMessageForEmbed(
    ...
    this.airlineFieldRef.nativeElement.remove();
    this.airlineLabelRef.nativeElement.remove();
  }
